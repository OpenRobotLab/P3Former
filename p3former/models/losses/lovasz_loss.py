...
class Lovasz_Softmax(torch.nn.Module):
    ...
    def __init__(self,ignore=0,loss_weight=1):
    ...
    def forward(self,
        ...
        if len(probas.shape) is 2:
            if self.ignore is not None:
                ...
                labels = labels[valid]
                if valid.sum()==0:
                    return valid.sum()*0.0
            if per_image:
                loss = mean(lovasz_softmax_flat(probas, labels, classes=classes)
                                for prob, lab in zip(probas, labels))
            else:
                loss = lovasz_softmax_flat(probas, labels, classes=classes)
        else:
            if per_image:
                loss = mean(lovasz_softmax_flat(*flatten_probas(prob.unsqueeze(0), lab.unsqueeze(0), self.ignore), classes=classes)
                                for prob, lab in zip(probas, labels))
            else:
                loss = lovasz_softmax_flat(*flatten_probas(probas, labels, self.ignore), classes=classes)
