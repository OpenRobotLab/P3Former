...
class PanopticEval:
  ...
  def reset(self):
    ...
    self.repeat = np.zeros([self.n_classes,100], dtype=np.int64)
    self.small = np.zeros([self.n_classes], dtype=np.int64)
    self.smallerror = np.zeros([self.n_classes], dtype=np.int64)
    self.map = []
    for i in range(self.n_classes):
      self.map.append(np.zeros([1,3], dtype=np.float))
  ...
  #############################  Panoptic STUFF ################################
  def addBatchPanoptic(self, x_sem_row, x_inst_row, y_sem_row, y_inst_row, fname):
    ...
    for cl in self.include:# stuff和things共同处理
      # ! 这里有一个小bug, stuff类road能从两个类映射过来，造成instance有两组，明天看看标准是怎么读取的 
      # print("*"*80)
      # print("CLASS", cl.item())
      # get a class mask
      ...
      tp_indexes = ious > 0.5 #计算所有ins_pred和ins_gt的IOU,取大于0.5的
      ...
      # if np.sum(np.logical_and(counts_gt >= self.min_points, matched_gt == False)) & (cl==1):
      #   print(fname,np.sum(np.logical_and(counts_gt >= self.min_points, matched_gt == False)),cl)
      ...
      #   import pdb
      #   pdb.set_trace()
      # if cl==8 and np.sum(np.logical_and(counts_gt >= self.min_points, matched_gt == False))>0:
      #   print('='*50)
      #   print('fname:',fname)
  ...
  def getPQ(self):
    ...
    rq_all = self.pan_tp.astype(np.double) / np.maximum(
        ...
        self.eps)
    print('tp:',self.pan_tp)
    print('fp:',self.pan_fp)
    print('fn:',self.pan_fn)
    pq_all = sq_all * rq_all

    print('repeat time:',self.repeat)
    ...
    # pdb.set_trace()
    # for i in range(20):
    #   np.save(f'visexp/map1k_{i}.npy',self.map[i])
  ...
  def addBatch_w_fname(self, x_sem, x_inst, y_sem, y_inst, fname, point=None):  # x=preds, y=targets
    ''' IMPORTANT: Inputs must be batched. Either [N,H,W], or [N, P]
    '''
    # add to IoU calculation (for checking purposes)
    self.addBatchSemIoU(x_sem, y_sem)
    ...
    # now do the panoptic stuff
    self.addBatchPanoptic(x_sem, x_inst, y_sem, y_inst, fname) #!

    self.evaluated_fnames.append(fname)

    return 'no sense' # match mmcv
