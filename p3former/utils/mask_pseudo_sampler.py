...
class MaskPseudoSampler(BaseSampler):
    """A pseudo sampler that does not do sampling actually."""#TODO: usage
    ...
    def sample(self, assign_result, masks, gt_masks, **kwargs):
        ...
        pos_inds = torch.nonzero(
            assign_result.gt_inds > 0, as_tuple=False).squeeze(-1).unique()#TODO why need unique()
...
class MyMaskSamplingResult(SamplingResult):
    """Bbox sampling result.
    ...
    def __init__(self, pos_inds, neg_inds, targets, assign_result):
        self.pos_inds = pos_inds
        self.neg_inds = neg_inds
        ...
        for key in targets:
            ...
            if key == 'masks':   
                if value.numel() == 0:
                    # hack for index error case
                    assert self.pos_assigned_gt_inds.numel() == 0
                    self.pos_gt_masks = torch.empty_like(value)
                    #self.pos_gt_masks = torch.empty_like(value)
                else:
                    self.pos_gt_masks = value[self.pos_assigned_gt_inds]
            if key == 'offsets':   
                if value.numel() == 0:
                    # hack for index error case
                    assert self.pos_assigned_gt_inds.numel() == 0
                    self.pos_gt_offsets = torch.empty_like(value)
                    #self.pos_gt_masks = torch.empty_like(value)
                else:
                    self.pos_gt_offsets = value[self.pos_assigned_gt_inds]
            if key == 'centers':   
                if value.numel() == 0:
                    # hack for index error case
                    assert self.pos_assigned_gt_inds.numel() == 0
                    self.pos_gt_centers = torch.empty_like(value)
                    #self.pos_gt_masks = torch.empty_like(value)
                else:
                    self.pos_gt_centers = value[self.pos_assigned_gt_inds]
...
class MyMaskPseudoSampler(BaseSampler): 
    ...
    def sample(self, assign_result, targets, **kwargs):
        """Directly returns the positive and negative indices  of samples.
        ...
        neg_inds = torch.nonzero(
            assign_result.gt_inds == 0, as_tuple=False).squeeze(-1).unique()
        sampling_result = MyMaskSamplingResult(pos_inds, neg_inds,
                                             targets, assign_result)
        return sampling_result
